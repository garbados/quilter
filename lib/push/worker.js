...
module.exports = function (job) {
  ...
  function update (task, callback) {
    log.info(task.id, 'checking remote update');
    ...
    db.get(task.id, function (err, doc) {
      ...
      if (doc.hash && hash === doc.hash) {
        log.info(task.id, '> rejected remote: identical hash');
        callback();
      } else if (task.timestamp >= doc.timestamp) {
        log.info(task.id, '> remotely updating');
        ...
        log.info(task.id, '> rejected remote: smaller timestamp');
        callback();
      }
    });
  }
